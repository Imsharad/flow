...
class TranscriptionManager: ObservableObject {
    ...
    func transcribe(buffer: AVAudioPCMBuffer, promptTokens: [Int]? = nil, prompt: String? = nil) async -> (text: String, tokens: [Int]?)? {
        ...
        let newTask = Task { () -> (text: String, tokens: [Int]?)? in
            ...
            do {
                let result = try await self.performTranscription(buffer: buffer, promptTokens: promptTokens, prompt: prompt)
                return result
    ...
    private func performTranscription(buffer: AVAudioPCMBuffer, promptTokens: [Int]?, prompt: String?) async throws -> (text: String, tokens: [Int]?) {
        // Check cancellation
        ...
        if currentMode == .cloud {
            do {
                ...
                return try await cloudService.transcribe(buffer, promptTokens: promptTokens, prompt: prompt)
            } catch {
        ...
        do {
            return try await localService.transcribe(buffer, promptTokens: promptTokens, prompt: prompt)
        } catch {
            throw error
        }
    }
