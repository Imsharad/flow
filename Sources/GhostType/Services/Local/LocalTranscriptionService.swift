...
actor LocalTranscriptionService: TranscriptionProvider {
    ...
    func transcribe(_ buffer: AVAudioPCMBuffer, prompt: String?) async throws -> (text: String, tokens: [Int]?) {
        lastAccessTime = Date()
        resetCooldownTimer()
        ...
        if let promptText = prompt, !promptText.isEmpty {
             // Tokenize the prompt text to provide context for the model
             promptTokens = await service.tokenize(text: promptText)
        }
        ...
        do {
            ...
            // Segments are not returned to DictationEngine yet but could be useful for chunking logic later
            _ = segments
            return (text, tokens)
        } catch {
