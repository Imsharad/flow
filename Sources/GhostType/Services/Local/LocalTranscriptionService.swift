...
actor LocalTranscriptionService: TranscriptionProvider {
    ...
    func transcribe(_ buffer: AVAudioPCMBuffer, promptTokens: [Int]? = nil, prompt: String? = nil) async throws -> (String, [Int]) {
        lastAccessTime = Date()
        resetCooldownTimer()
        ...
        do {
            let (text, tokens, _) = try await service.transcribe(audio: floatArray, promptTokens: promptTokens, prompt: prompt)
            return (text, tokens)
        } catch {
